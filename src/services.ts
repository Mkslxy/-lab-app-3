...
export class LibraryService {
    ...
    constructor() {
        ...
        if (savedBooks) savedBooks.forEach(book => this.books.add(new Book(book.title, book.author, book.year, book.isBorrowed)));

        const savedUsers = Storage.load<User[]>('users');
        if (savedUsers) savedUsers.forEach(user => this.users.add(new User(user.name, user.email)));
    }
